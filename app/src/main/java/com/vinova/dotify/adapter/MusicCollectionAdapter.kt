...
class MusicCollectionAdapter(private val context: Context, private val type: Int) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (type) {
            1 ->  (holder as SuggestedPlaylistHolder).bind(listCollection.elementAt(position))
            2 -> (holder as GenrePlaylistHolder).bind(listCollection.elementAt(position))
            else -> (holder as FeaturedAlbumHolder).bind(listCollection.elementAt(position))
        }
